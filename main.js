...
function deleteTask(e) {
  if (e.target.matches(".trash-task")) {
    ...
    const targetDate =
      e.target.parentNode.querySelector(".task-date").textContent;
    const targetTime =
      e.target.parentNode.querySelector(".task-time").textContent;
    const targetNotes =
      e.target.parentNode.querySelector(".task-notes").textContent;
...
function showAllProjects() {
  ...
  Object.keys(toDoList)
    .filter((project) => project !== "Home Tasks")
...
function markIncomplete(e) {
  const targetTitle =
    e.target.parentNode.parentNode.querySelector("strong").textContent;
  const targetDate =
    e.target.parentNode.parentNode.querySelector(".task-date").textContent;
  const targetTime =
    e.target.parentNode.parentNode.querySelector(".task-time").textContent;
  const targetNotes =
    e.target.parentNode.parentNode.querySelector(".task-notes").textContent;
...
function getCurrentTasks() {
  ...
  allAddedTasks.forEach((element) => {
    ...
    const { priority } = element.querySelector(".label-box").dataset;
    const { completed } = element.dataset;
...
buttonMenuOpen.addEventListener("click", () => {
  ...
  container.style.gridTemplateColumns =
    "calc(15vw + 40px) auto calc(15vw + 40px)";
